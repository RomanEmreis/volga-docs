...
::: warning
**Global** means every incoming request, including the ones that match no route — which is most of what a scanner or a naive flood sends. Size the budget against real traffic rather than against the route count, and give middleware that should skip unmatched requests a [`matched_route()`](https://docs.rs/volga/latest/volga/middleware/struct.HttpContext.html#method.matched_route) check.
:::
