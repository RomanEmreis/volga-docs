...
The simplest option — automatically discovers `app_config.toml` or `app_config.json` in the current working directory:
...
::: warning
`reload_on_change()` requires `with_file()` to be called. The app will panic at startup if no file path is configured.
:::
