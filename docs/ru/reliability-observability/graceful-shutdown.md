...
Когда приходит сигнал завершения, цикл приёма перестаёт принимать новые соединения, а `run()` дожидается, пока открытые соединения доработают то, что обслуживают, — но не дольше **10 секунд**, после чего перестаёт ждать и возвращает управление. Сервисы приложения, включая синглтоны, освобождаются после этого ожидания. Незавершённое TLS-рукопожатие не ждут, а обрывают: запроса за ним ещё нет.

::: warning Исправлено в 0.10.1
Плавное завершение пропускалось для **всех** открытых соединений, если в момент остановки цикла приёма хоть один запрос был в обработке, — то есть в самом обычном случае, ведь именно это завершение обычно и прерывает. `run()` возвращал управление, пока ответы ещё записывались, и их обрывало то, что следующим уронит рантайм, — а под [`run_blocking()`](https://docs.rs/volga/latest/volga/app/struct.App.html#method.run_blocking) это конец `main`.

В коде приложения из-за этого менять нечего: rolling-деплой просто перестаёт терять запросы, которые уже обслуживал.
:::
