...
## Когда токен отменяется

Токен запроса принадлежит его соединению: он отменяется, когда соединение обрывается — клиент ушёл, — и когда у [плавного завершения](/volga-docs/ru/reliability-observability/graceful-shutdown.html#таимаут-завершения) истекает таймаут и соединение закрывается. Только что начавшаяся остановка его не отменяет, поскольку запросы в обработке всё ещё получают ответ; обработчик, который должен сворачиваться вместе с сервером, принимает [`ShutdownHandle`](/volga-docs/ru/reliability-observability/graceful-shutdown.html#завершение-долгих-ответов).

Обработчик уничтожается вместе со своим соединением, поэтому токен важнее всего для работы, которая переживает обработчик: запущенной им задачи или тела [`blocking`](/volga-docs/ru/getting-started/handlers.html#отмена), проверяющего `is_cancelled()` по ходу работы. Отмена токена из обработчика оповещает его клоны и ничего не закрывает.
