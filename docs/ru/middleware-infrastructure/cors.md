...
Ответы `404` и `405` несут те же заголовки CORS, которые настроенная политика проставила бы на `200`: глобальный middleware выполняется и для запросов, не совпавших ни с одним маршрутом, поэтому браузер получает осмысленную ошибку CORS вместо непрозрачной.
