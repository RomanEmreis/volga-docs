...
:::warning  
Если вам нужно включить передачу учётных данных с помощью [`with_credentials()`](https://docs.rs/volga/latest/volga/http/cors/struct.CorsConfig.html#method.with_credentials), учтите, что **нельзя** использовать эту настройку совместно с wildcard-значениями для источников, заголовков или методов по соображениям безопасности. Эти ограничения проверяются в [`use_cors()`](https://docs.rs/volga/latest/volga/app/struct.App.html#method.use_cors), и в случае некорректной конфигурации произойдёт паника. Метод не принимает аргументов — для явного отключения используйте [`without_credentials()`](https://docs.rs/volga/latest/volga/http/cors/struct.CorsConfig.html#method.without_credentials). Аналогично, заголовок `Vary` управляется через [`with_vary_header()`](https://docs.rs/volga/latest/volga/http/cors/struct.CorsConfig.html#method.with_vary_header) / [`without_vary_header()`](https://docs.rs/volga/latest/volga/http/cors/struct.CorsConfig.html#method.without_vary_header).
:::
...
## CORS для несовпавших запросов

Начиная с **0.10.0** ответы `404` и `405` несут те же заголовки CORS, которые настроенная политика проставила бы на `200`. Глобальный middleware теперь выполняется и для запросов, не совпавших ни с одним маршрутом, поэтому браузер получает осмысленную ошибку CORS вместо непрозрачной.

На **preflight** по-прежнему отвечают `204` только для существующего маршрута — ответить так на путь или метод, которые роутер не нашёл, значило бы объявить эндпоинт, которого нет. Такие запросы проваливаются в `404` / `405` и забирают заголовки на выходе. Middleware CORS различает эти случаи через [`matched_route()`](https://docs.rs/volga/latest/volga/middleware/struct.HttpContext.html#method.matched_route).

::: warning Статические файлы
Политика CORS [группы маршрутов](/volga-docs/ru/getting-started/route-groups.html) до раздаваемых этой группой файлов не доходит. Политика выбирается по сработавшему маршруту, а статический файл раздаётся без маршрута, поэтому к файлам применяется политика **приложения** — заданная через [`with_cors()`](https://docs.rs/volga/latest/volga/app/struct.App.html#method.with_cors). По той же причине preflight, нацеленный на путь файла, обрабатывается как запрос к несуществующему пути: `404` с заголовками политики, а не `204`. Кросс-доменные `GET` к статике его всё равно никогда не отправляют.
:::
