...
**Глобально** теперь действительно означает все входящие запросы, включая те, что не совпали ни с одним маршрутом. До 0.10.0 глобальный конвейер middleware запускался только из одной ветки диспетчера, поэтому `use_token_bucket(by::ip())` обходился простым запросом к несуществующему пути — именно так по умолчанию ведёт себя сканер или наивный флуд. А в приложении, где fallback раздаёт SPA-оболочку, каждый такой неучтённый запрос был чтением с файловой системы плюс полное HTML-тело вместо дешёвого `404`.

Бюджет лимитера теперь расходуется трафиком, который раньше его не касался, поэтому **сервис, рассчитанный по своим же маршрутам, может увидеть, что клиенты упираются в лимит раньше, чем прежде**. Middleware, который должен пропускать несовпавшие запросы, может проверить [`matched_route()`](https://docs.rs/volga/latest/volga/middleware/struct.HttpContext.html#method.matched_route).

Ограничение частоты, назначенное **группе маршрутов**, теперь также доходит до всех зарегистрированных группой маршрутов, в каком бы порядке это ни было написано внутри замыкания — раньше маршрут, замапленный до `token_bucket` своей группы, вёл себя так, будто его нет.
:::
