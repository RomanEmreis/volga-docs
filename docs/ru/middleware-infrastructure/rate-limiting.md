...
::: warning
**Глобально** означает все входящие запросы, включая те, что не совпали ни с одним маршрутом, — а именно такие в основном и шлёт сканер или наивный флуд. Рассчитывайте бюджет по реальному трафику, а не по количеству маршрутов, а middleware, который должен пропускать несовпавшие запросы, снабдите проверкой [`matched_route()`](https://docs.rs/volga/latest/volga/middleware/struct.HttpContext.html#method.matched_route).
:::
