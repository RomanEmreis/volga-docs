...
Раньше поиск фиксировался на первом же совпавшем литерале и назад не возвращался, поэтому `/files/{name}` переставал отвечать на `/files/shared`, как только рядом появлялся `/files/shared/latest`. Работавший маршрут начинал отдавать `404` из-за добавленного рядом более длинного — при том, что по самим шаблонам ничто не говорит, что они как-то связаны.
:::
