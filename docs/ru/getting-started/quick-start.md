...
:::warning
Непригодный адрес сообщается явно, а не угадывается: [`run()`](https://docs.rs/volga/latest/volga/app/struct.App.html#method.run) возвращает `io::Error`, а [`run_blocking()`](https://docs.rs/volga/latest/volga/app/struct.App.html#method.run_blocking) логирует ошибку и не запускает сервер. Подмены на `0.0.0.0:7878` не происходит, поэтому привязка к loopback остаётся на loopback.
:::
...
Обработчику, которому нечего ждать, можно возвращать ответ напрямую, как этот, а обработчик, который чего-то ждёт, — это `async`-замыкание или `async fn`. Обе формы описаны в разделе [Обработчики](/volga-docs/ru/getting-started/handlers.html).
